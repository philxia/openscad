...
int main(int argc, char **argv)
{
  ...
  for (const auto &name : ColorMap::inst()->colorSchemeNames()) {
    if (!colorSchemeNames.empty()) colorSchemeNames.append(" | ");
    if (name == ColorMap::inst()->defaultColorSchemeName()) colorSchemeNames.append("*");
  ...
  for (auto it = Feature::begin(); it != Feature::end(); ++it) {
    if (!features.empty()) features.append(" | ");
