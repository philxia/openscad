...
void MainWindow::updateCamera(const FileContext &ctx)
{
  ...
  if (vpr->getVec3(x, y, z)) qglview->cam.setVpr(x, y, z);

  const auto vpt = ctx.lookup_variable("$vpt");
  if (vpt->getVec3(x, y, z)) qglview->cam.setVpt(x, y, z);

  const auto vpd = ctx.lookup_variable("$vpd");
  if (vpd->type() == Value::ValueType::NUMBER) qglview->cam.setVpd(vpd->toDouble());
}
